...
const install = async () => {
    ...
    await terminal(`apt update` , spin)
        .then(([res , err]) => {
            if(!err) throw new Error(res)
        }).catch(err => {throw new Error(err)})
    await terminal(`apt upgrade -y` , spin)
        .then(([res , err]) => {
            if(!err) throw new Error(res)
        }).catch(err => {throw new Error(err)})
    await terminal(`apt install curl build-essential git wget jq make gcc tmux chrony -y` , spin)
        .then(([res , err]) => {
            if(!err) throw new Error(res)
        }).catch(err => {throw new Error(err)})
